    ...
    let app = createApp(
      {
        endpoints: {
          http: {
            "/hello": ["error,500,This is an error"],
          },
        },
      },
      logger,
    ...
    let app = createApp(
      {
        endpoints: {
          http: {
            "/hello": ["log,warn,This is a log message"],
          },
        },
      },
      logger,
  ...
  it("returns some HTML based on the configuration", (t, done) => {
    ...
    supertest(app)
      ...
      .then((response) => {
        assert.strictEqual(
          response.text,
          "[\"<img src='/image.jpg' />\",\"<script src='/script.js?output=javascript'></script>\",\"<script>var o = new XMLHttpRequest();o.open('GET', '/ajax.json');o.send();</script>\"]",
        );
        done();
      });
  });
  it("connects with a remote service for HTTP calls", (t, done) => {
    ...
    let app = createApp(
      {
        endpoints: {
          http: {
            "/hello": ["http://example/hello", "https://mockservice/echo"],
          },
        },
      },
      logger,
    ...
    supertest(app)
      ...
      .then((response) => {
        ...
        assert.strictEqual(
          response.text,
          '["Hello from example","Hello from example"]',
        );
        done();
      });
  });
  it("skips a call that has a 0% probability", (t, done) => {
    let app = createApp(
      {
        endpoints: {
          http: {
    ...
    supertest(app)
      ...
      .then((response) => {
        assert.strictEqual(
          response.text,
          '["error,500,not available was not probable"]',
        );
        done();
      });
  });
