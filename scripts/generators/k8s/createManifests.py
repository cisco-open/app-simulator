            ...
            for service, config in value.items():
                ...
                if config['type'] in application_services:
                    ...
                    if config.get('port') != None:
                        write_yaml(f"./deployments/{service}-service-ext.yaml", renderService(templ='./templates/service.yaml.tmpl',service_name=service,service_port=8080,service_ext_port=config.get('port'),service_type="ext"))
                    write_yaml(f"./deployments/{service}-service.yaml",renderService(templ='./templates/service.yaml.tmpl',service_name=service,service_port=8080,service_ext_port=8080,service_type="int"))
                    write_yaml(f"./deployments/{service}-configmap.yaml",renderConfigMap(templ='./templates/config-map.yaml.tmpl',service_name=service, service_config=config))
                    write_yaml(f"./deployments/{service}-deployment.yaml",renderDeployment(templ='./templates/deployment.yaml.tmpl', service_name=service,service_type=config.get('type'),service_port=8080))
                elif config['type'] in db_services:
                    print(f"create DB Service of type {config['type']} named {service}")
                    write_yaml(f"./deployments/{service}-service.yaml",renderService(templ='./templates/db-service.yaml.tmpl',service_name=service,service_port=0,service_ext_port=0,service_type=config['type']))
                    write_yaml(f"./deployments/{service}-configmap.yaml",renderConfigMap(templ='./templates/config-map.yaml.tmpl',service_name=service, service_config=config))
                    write_yaml(f"./deployments/{service}-deployment.yaml",renderDeployment(templ='./templates/db-deployment.yaml.tmpl', service_name=service,service_type=config.get('type'),service_port=8080))
                else:
